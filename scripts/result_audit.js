...
const sourceAsAuthority = results.filter(r => {
  ...
  return s && r.authority && String(r.authority).includes(s.title || '__NO_MATCH__');
});
...
const report = {
  ...
  qa:{broken_next_id:broken.length,unreachable_questions:unreachableQuestions.length,unreachable_results:unreachableResults.length,dead_ends:deadEnds.length,missing_source_id:missingSource.length,duplicate_questions:duplicateQuestions.length,duplicate_options:duplicateOptions.length,results_without_action:noAction.length,results_without_authority:noAuthority.length,expert_bureau_as_authority:falseAuthority.length,source_title_in_authority:sourceAsAuthority.length},
