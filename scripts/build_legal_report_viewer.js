...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:var(--bg);color:var(--ink);font-family:Tahoma,Arial,sans-serif;line-height:1.7}
    ...
    .pager{display:flex;justify-content:center;align-items:center;gap:.7rem;margin:1rem}.pager button{min-height:44px;padding:.5rem 1rem;border:1px solid var(--green);background:#fff;color:var(--green);border-radius:10px}.pager button:disabled{opacity:.4}#results:focus{outline:3px solid var(--gold);outline-offset:6px}
...
<body>
  ...
  <main>
    ...
    <section class="toolbar" aria-label="أدوات التقرير">
      ...
      <div class="meta"><strong id="count" aria-live="polite"></strong><div class="actions"><a class="btn" href="../../docs/LEGAL_APPROVAL_MASTER.csv" download>تنزيل CSV الأصلي</a><a class="btn" href="../../docs/LEGAL_APPROVAL_EXECUTIVE_SUMMARY.md">الملخص التنفيذي</a><button id="print" class="btn" type="button">طباعة النتائج المصفّاة</button></div></div>
    </section>
    <section id="results" class="cards" aria-label="نتائج التقرير" tabindex="-1"></section>
  ...
  <script>
    ...
    function cardMarkup(x,open=false){return '<details class="card"'+(open?' open':'')+'><summary><span class="id">'+escapeHtml(x['Result ID'])+'</span><span class="sector">'+escapeHtml(x['القطاع'])+'</span><span class="risk '+escapeHtml(x['مستوى الخطورة القانونية'])+'">'+escapeHtml(x['مستوى الخطورة القانونية'])+'</span></summary><dl class="content">'+FIELDS.map(f=>'<div class="field"><dt>'+escapeHtml(f)+'</dt><dd>'+linkify(x[f],f)+'</dd></div>').join('')+'</dl></details>'}
    function render(){const pages=Math.max(1,Math.ceil(filtered.length/PAGE_SIZE));page=Math.min(page,pages);const items=filtered.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);el('count').textContent='المعروض: '+filtered.length+' من '+DATA.length+' نتيجة';el('page').textContent='صفحة '+page+' من '+pages;el('prev').disabled=page===1;el('next').disabled=page===pages;el('results').innerHTML=items.length?items.map(x=>cardMarkup(x)).join(''):'<p class="empty">لا توجد نتائج مطابقة.</p>'}
    function printReport(){const pager=document.querySelector('.pager'),savedPage=page;el('results').innerHTML=filtered.length?filtered.map(x=>cardMarkup(x,true)).join(''):'<p class="empty">لا توجد نتائج مطابقة.</p>';pager.hidden=true;window.print();pager.hidden=false;page=savedPage;render()}
    ['search','risk','verification'].forEach(id=>el(id).addEventListener(id==='search'?'input':'change',apply));el('prev').addEventListener('click',()=>{page--;render();scrollTo({top:0,behavior:'smooth'})});el('next').addEventListener('click',()=>{page++;render();scrollTo({top:0,behavior:'smooth'})});el('print').addEventListener('click',printReport);render();
  </script>
</body>
</html>`;
